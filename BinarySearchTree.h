...
public:
	...
	bool insert(T arg, BST * parent = NULL)
	{
		if (empty)
		{
			value = arg;
	...
	bool remove(T arg, BST * parent = NULL)
	{
	...
	// Overloaed equality - All nodes contain the same elements in the same positions
...
// Overloaded Stream Insertion Operator
template <typename T>
ostream & operator<<(ostream& output, BST<T> arg)
{
	arg.printBFS();
	return output;	
}
