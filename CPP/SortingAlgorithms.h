...
void SelectionSort(T * array, int size) {
	...
	for (int i = 0; i < size - 1; i++) {
		...
		// Adjust to get absolute position
		currentLargest = i + findMax( &(array[i]), size - i );	
		// cout << currentLargest << " ";
...
void Merge(T * array, int aBegin, int aEnd, int bBegin, int bEnd) {
	// beginning position of the current part of the actual array
	int arrayBegin = aBegin;	
	int totalArraySize = (aEnd - aBegin + 1) + (bEnd - bBegin + 1);
	// current position in tempArray
	int currentPosition = 0;	
...
void QuickSort(T * array, int start, int end) {
	...
	// The index to swap elements into
